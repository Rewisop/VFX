...
const replicateImageOutputSchema = z
  .object({
...
const replicatePredictionSchema = z.object({
  ...
  output: z
    .union([
      z.string(),
      replicateImageOutputSchema,
      z.array(z.union([z.string(), replicateImageOutputSchema])),
      z.null(),
      z.undefined(),
    ])
    .optional(),
...
function extractReplicateResult(
  ...
  if (Array.isArray(output)) {
    for (const item of output) {
      ...
      if (item && typeof item === "object") {
        ...
        if (
          "image_base64" in item &&
          typeof item.image_base64 === "string" &&
          item.image_base64.length > 0
        ) {
  ...
  if (typeof output === "object") {
    ...
    if (
      "image_base64" in output &&
      typeof output.image_base64 === "string" &&
      output.image_base64.length > 0
    ) {
